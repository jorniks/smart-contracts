yarn run v1.22.19
$ /opt/smart-contracts/projects/hashsend/node_modules/.bin/hardhat flatten contracts/HashSend.sol
// Sources flattened with hardhat v2.22.19 https://hardhat.org

// SPDX-License-Identifier: MIT

// File contracts/HashSend.sol
...
contract HashSend {
  ...
  /// @param _newRecipient The address that will receive the tokens (allows recipient to change destination).
  ...
  function claimToken(
    ...
    require(block.timestamp <= txRecord.expiration, "Transaction expired");
      ...
      // If an encrypted password exists, verify it against the provided password.
    ...
    // Update transaction details with new parameters provided by the claimer
    txRecord.recipient = _newRecipient;
    txRecord.destinationChain = _newDestinationChain;
    // If new destination token is not provided (i.e., address(0)), keep the original destinationToken
    if (_newDestinationToken != address(0)) {
        txRecord.destinationToken = _newDestinationToken;
      }
    txRecord.status = TxStatus.Claimed;

    // Transfer tokens from the contract to the new recipient
    require(IERC20(txRecord.token).transfer(_newRecipient, txRecord.amount), "Token transfer failed");

    // Update sender totals (claimed)
